...
var chart_heatmap_large, chart_heatmap_color, chart_dias, chart_horas, app, chart_relogio_tarde, chart_historico;
...
function ajustarTextos(){
	$("#denseTexto").html("Dense Display Pixel");
	$("#heatmapTexto").html("Heatmap Semanal");
	$("#diasHorasTexto").html("Dias / Horas");
	$("#relogioTexto").html("Relógio");
}
...
function abreDados(ano, lugar){
	...
	heatmap_large = {
		...
		$.getJSON(file, function(data){
			...
			$.each(data, function (key, val) {
				if (key != 0) {
					...
					/*if(contador_historico == 0){
						data_historico = dataUTC;
					}
					
					valor_historico += valor;
					contador_historico++;
					
					if(contador_historico == 23){
						conteudoHistorico.push([dataUTC, valor_historico]);
						dataUTC = "";
						valor_historico = 0;
					...
					// primeira linha
					if (null == $("#energia").val()){
						$('#energia').append($("<option></option>").attr("value","Média").text("Média")); 
			...
			$("#denseTexto").html("Dense Display Pixel - Fonte selecionada: <b>" + fonte + "</b> - Consumo máximo: <b>" + parseInt(maxDenseDisplay) + "</br>");
...
function carregaHeatmap(heatmapcolor, lista_itens, maxDenseDisplay){
	...
	heatmapcolor.xAxis.categories = ['Segunda<br><b>' + formatarData(lista_dias[0]), 
									 'Terça<br><b>' + formatarData(lista_dias[1]),  
									 'Quarta<br><b>' + formatarData(lista_dias[2]),  
									 'Quinta<br><b>' + formatarData(lista_dias[3]),  
									 'Sexta<br><b>' + formatarData(lista_dias[4]),  
									 'Sábado<br><b>' + formatarData(lista_dias[5]),  
									 'Domingo<br><b>' + formatarData(lista_dias[6])];
									 
	dataInicioSemana = formatarData(lista_dias[0]);
	dataFimSemana = formatarData(lista_dias[6]);
	
	$("#heatmapTexto").html("Heatmap Semanal - Semana selecionada: <b>" + semana_selecionada + "</b> - Inicio: <b>" + dataInicioSemana + "</b> - Fim: <b>" + dataFimSemana + "</b>");
